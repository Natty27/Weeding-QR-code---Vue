<template>
  <!--
    Official ChiNet mark. Source file: src/assets/chinet-icon.svg
    viewBox is trimmed to the path's real bounds (the source file leaves room
    for the wordmark and a "TM" that is unreadable at brand-mark sizes), and the
    fill follows `currentColor` so the lockup can be white on dark backgrounds
    or brand blue (#22f) on light ones.
  -->
  <svg
    class="chinet-mark"
    viewBox="0 7.5 1585.79 1126.41"
    :width="width"
    :height="height"
...
/**
 * The mark is 1.41:1, so it is sized by height and the width follows.
 * Height is applied inline on purpose: a scoped class here and a class on the
 * consuming element have the same specificity, and whichever the bundler emits
 * last would win — which made the mark fill its container in dev builds.
 */
const props = defineProps({
  height: { type: Number, default: 38 },
});

const ASPECT = 1585.79 / 1126.41;
