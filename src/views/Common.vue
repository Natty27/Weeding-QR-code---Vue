<template>
  <div class="page">
    ...
    <header class="hero">
      ...
      <p>Create elegant QR invitations for your commons</p>
    </header>
      ...
      <section class="form-card">
        ...
        <div class="input-group">
          <input
            ...
            @keyup.enter="addCommon"
          />
          <button @click="addCommon" :disabled="isCreating">
    ...
    <transition-group name="fade-up" tag="section" class="grid">
      <div v-for="g in commons" :key="g._id" class="common-card">
...
const bulkGenerate = async () => {
  ...
  await api.post("/common/bulk", { count: bulkCount.value });
  await load();
...
const addCommon = async () => {
  ...
  isCreating.value = true;
  await api.post("/common", { name: name.value || null });
  name.value = "";
  await load();
  isCreating.value = false;
};
...
const load = async () => {
  ...
  commons.value = res.data.reverse();
